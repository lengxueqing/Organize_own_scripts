...
sub prepareCosmic {
	...
	while (<VCF>) {
		...
		my ($chr, $start, $id2, $ref_allele, $mut_allele) = @field[0..4];
		my $end = $start + length($ref_allele) - 1;

        
        
        my $id="";
       my @info=split/;/,$field[-1];
       for my $kk(@info){
            my ($tag,$value)=split/=/,$kk;
            if($tag eq "LEGACY_ID"){
                $id=$value;
            }
       }
   ...
   ## print Dumper(\%mut);
	...
	if ($field[17] eq 'LEGACY_MUTATION_ID') {
		$mutid = $field[17];
		$filetype = 'coding';
	} elsif ($field[12] eq 'LEGACY_MUTATION_ID') {
		$mutid = $field[12];
		$filetype = 'noncoding';
	} else {
